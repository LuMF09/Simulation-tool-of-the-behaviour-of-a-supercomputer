...
void Scheduler::startScheduler(int nbtime)
{
    ...
    if(total==0)
    {
        throw DivideZero("Check total value in Scheduler function: startScheduler");
    }
...
std::vector<double> Scheduler::getParametersDouble()
{
    std::vector<double> res;
    res.push_back(this->operatingCost);
    res.push_back(this->turnaroundTime);
    res.push_back(this->totPersonalResource);
    res.push_back(this->totalRequestShort);
    res.push_back(this->totalRequestMedium);
    res.push_back(this->totalRequestLarge);
    res.push_back(this->totalRequestHuge);
    res.push_back(this->nbMH);

    return res;
}

Machine Scheduler::getMachine()
{
    return machine;
}
